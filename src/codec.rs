    ...
    match codec {
        ...
        VectorCodec::ForBitpack => {
            ...
            shifted
                ...
                .map(|v| {
                    v.checked_add(min)
                        .ok_or(GoweError::InvalidData("u64 FOR overflow"))
                })
                .collect()
        }
...
fn decode_u64_direct_bitpack(reader: &mut Reader<'_>) -> Result<Vec<u64>> {
    ...
    if width == 0 || width > 64 {
        return Err(GoweError::InvalidData("bitpack width"));
    }
...
fn decode_u64_simple8b_inner(reader: &mut Reader<'_>) -> Result<Vec<u64>> {
    ...
    if mode != 1 {
        return Err(GoweError::InvalidData("simple8b mode"));
    }
    ...
    while out.len() < len {
        ...
        match selector {
            ...
            _ => return Err(GoweError::InvalidData("simple8b selector")),
        }
    }
    Ok(out)
}
...
fn undelta(values: Vec<i64>) -> Result<Vec<i64>> {
    ...
    for (idx, value) in values.into_iter().enumerate() {
        ...
        let next = prev
            .checked_add(value)
            .ok_or(GoweError::InvalidData("delta overflow"))?;
        out.push(next);
        prev = next;
    }
    Ok(out)
    ...
    for _ in 1..len {
        ...
        let bits = if flag == 0 {
            ...
            if leading + trailing + width > 64 {
                return Err(GoweError::InvalidData("xor-float bit widths"));
            }
            let x = if width == 64 {
                payload
            } else {
...
fn decode_i64_delta_delta(reader: &mut Reader<'_>) -> Result<Vec<i64>> {
    ...
    if dd.len() != len.saturating_sub(2) {
        return Err(GoweError::InvalidData("delta-delta length"));
    }
    let mut out = Vec::with_capacity(len);
    out.push(first);
    let mut prev = first;
    let second = prev
        ...
        .ok_or(GoweError::InvalidData("delta-delta overflow"))?;
    out.push(second);
    prev = second;
...
fn unpack_u64_values(reader: &mut Reader<'_>, len: usize, width: u8) -> Result<Vec<u64>> {
    ...
    for _ in 0..len {
        while acc_bits < width as u32 {
            let b = *bytes
                .get(idx)
                .ok_or(GoweError::InvalidData("bitpack underflow"))?;
            idx += 1;
